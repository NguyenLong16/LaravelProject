...
class LichChieuController extends Controller
{
    ...
    {
        $lichChieus = LichChieu::with(['movie', 'room']) // Lấy cùng với 'movie' và 'room'  
        ->where('status', 0) // Lọc lịch chiếu có trạng thái 0  
        ->get();  

    // Thêm số ghế có sẵn vào từng lịch chiếu  
        foreach ($lichChieus as $lichChieu) {  
            // Đếm số ghế có STATUS = 0 từ bảng TrangThaiGhe  
            $lichChieu->available_seats_count = TrangThaiGhe::where('IDLICHCHIEU', $lichChieu->IDLICHCHIEU)  
                ->where('STATUS', 0)  
                ->count(); // Đếm ghế có trạng thái = 0  
        }  
